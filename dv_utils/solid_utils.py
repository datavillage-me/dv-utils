...
def get_uma_token(solid_idp_token: str, resource_uri: str, access_grant: dict):
  ...
  uma_unscoped_token = __request_uma_unscoped_token(solid_idp_token, uma_token_endpoint, permission_ticket) 

  # Call 4: get uma token with scopes
  return __request_uma_scoped_token(uma_token_endpoint, permission_ticket, access_grant, uma_unscoped_token['access_token'])
...
def __request_uma_scoped_token(uma_token_endpoint: str, permission_ticket: str, access_grant: dict, unscoped_token: str) -> str:
  ...
  payload = {
    ...
    'grant_type': 'urn:ietf:params:oauth:grant-type:uma-ticket',
    'claim_token': vp_b64,
    'claim_token_format': f"https://www.w3.org/TR/vc-data-model/#json-ld&rpt={unscoped_token}"
  }
